...
public class RecruitmentFormController {
    ...
    public String selectCourseType2(Model model, int tid) {
        model.addAttribute("tid", tid);
        return "courseTypeSelect2";
    }

    @RequestMapping(value="/recruitmentForm.html", params = {"tid", "rid"}, method = RequestMethod.GET)
    public String showRecruitmentForm(Model model, int tid, int rid, HttpServletRequest request) {
    ...
    public String processRecruitmentForm(@Valid @ModelAttribute("rekrutacja")Recruitment recruitment, BindingResult bindingResult, Principal principal) {
    ...
    public String showRecruitmentForm2(Model model, int tid, int rid, HttpServletRequest request) {
        pom = request.getQueryString();
        if(tid==1) {
            if(rid==2) {
                model.addAttribute("kursy", courseService.getFilteredCourses("Stacjonarne", "Magisterskie"));
                courseService.getFilteredCourses("Stacjonarne","Magisterskie").size();
                courseService.getFilteredCourses("Stacjonarne","Magisterskie").size();
            }
    ...
    public String processRecruitmentForm2(@ModelAttribute("rekrutacja") Recruitment recruitment, Principal principal) {
